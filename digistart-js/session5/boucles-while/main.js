...
while (next) {
    ...
    next = nombres[i] != 29 && i + 1 < nombres.length;
    i++;
}
...
i = 0;
while (i < nombres.length && nombres[i] < 50) {
    console.log(nombres[i]);
    i++;
}
...
i = 0;
while (i < prenoms.length && prenoms[i].length <= 5) {
...
i = 0;
while (i < prenoms.length && (prenoms[i].toLowerCase().includes("a"))) {
    console.log(prenoms[i]);
    i++;
}
